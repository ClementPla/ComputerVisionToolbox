...
import { addMatrices, matmul, inverse, transpose, MatrixClass } from './matrix';
...
export function polyfit_ridge(
  ...
  const n = x.length;
  const phi: MatrixClass = new MatrixClass(n, degree + 1);
  const t: MatrixClass = new MatrixClass(n, 1);
  for (let i = 0; i < n; i++) {
  ...
  const phiTphi = phiT.multiply(phi);
  const lambdaI = MatrixClass.identity(degree + 1).map((row) =>
    row.map((val) => val * lambda)
  );
  const toInvert = addMatrices(phiTphi, lambdaI);
  const inverted = inverse(toInvert);
  if (inverted === null) {
    throw new Error('Matrix inversion failed in ridge regression.');
  }
  const phiTt = matmul(phiT, t);
...
export function polyfit_gradientDescent(
  ...
  const n = x.length;
  let initialEstimate = polyfit(x, y, degree);
  let coeff: MatrixClass = new MatrixClass(degree + 1, 1);
  for (let j = 0; j <= degree; j++) {
    coeff[j][0] = initialEstimate[j];
  }
  const phi: MatrixClass = new MatrixClass(n, degree + 1);
  const t: MatrixClass = new MatrixClass(n, 1);
  ...
  let epsilon = 1e-7;
  // Gradient descent
  let previous_coeff = coeff.map((row) => row[0]);
  let previous_loss = Infinity;

  for (let iter = 0; iter < iterations; iter++) {
    const errors = phi.multiply(coeff).subtract(t);
    const loss =
      errors.transpose().multiply(errors)[0][0] +
      lambda * coeff.abs().addScalar(epsilon).scalarPower(q).sum();
    if (Math.abs(previous_loss - loss) < 1e-6) {
      break;
    }
    previous_loss = loss;
    previous_coeff = coeff.map((row) => row[0]);
    // If the new loss is worse, reduce the learning rate
    ...
    let gradient = phi.scalarMultiply(2).transpose().multiply(errors);

    gradient = gradient.add(
      coeff
        .sign()
        .scalarMultiply(lambda * q)
        .hadamard(
          coeff
            .abs()
            .addScalar(epsilon)
    ...
    if (gradNorm > 1e3) {
      gradient = gradient.scalarMultiply(1e3 / gradNorm);
    }

    coeff = coeff.subtract(gradient.scalarMultiply(learningRate));
  }
  return coeff.map((row) => row[0]);
}
export function polyfit_regularized(
  x: number[],
  y: number[],
  degree: number,
  lambda: number,
  ...
  if (q == 2) {
    return polyfit_ridge(x, y, degree, lambda);
  }

  return polyfit_gradientDescent(x, y, degree, lambda, q);
}
