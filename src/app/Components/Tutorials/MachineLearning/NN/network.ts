  ...
  swap_fc_layers(position: number, new_layer: FullyConnected) {
    ...
    for (let i = 0; i < this.layers.length; i++) {
      ...
      if (current_layer instanceof FullyConnected) {
        if (n === position) {
          let min_dim = Math.min(
            current_layer.weights.shape[0],
            new_layer.weights.shape[0]
          );
          let min_dim2 = Math.min(
            current_layer.weights.shape[1],
            new_layer.weights.shape[1]
          );
          for (let j = 0; j < min_dim; j++) {
            for (let k = 0; k < min_dim2; k++) {
              new_layer.weights.data[j * new_layer.weights.shape[1] + k] =
                current_layer.weights.data[
                  j * current_layer.weights.shape[1] + k
                ];
              if (new_layer.bias) {
