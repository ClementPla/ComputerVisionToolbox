...
export class Trainer {
  ...
  losses: number[] = [];
  trainTimeout: NodeJS.Timeout | undefined;
  ...
  changeOptimizer(optimizer: string, lr: number, weight_decay: number) {
    if (optimizer === 'adam') {
      this.optimizer = new Adam(this.model, lr, weight_decay, 0.1);
    } else if (optimizer === 'sgd') {
      this.optimizer = new SGD(this.model, lr, weight_decay, 0.1);
    } else {
      this.optimizer = new RMSProp(this.model, lr, weight_decay, 0.1);
    }
