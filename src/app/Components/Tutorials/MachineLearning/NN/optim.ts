    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (this.momentums && this.velocities) {
        ...
        this.velocities[i] = this.velocities[i].map((velocity, j) => {
          return (
            this.beta2 * velocity +
            (1 - this.beta2) * param.grad[j] * param.grad[j]
          );
        });
        ...
        for (let j = 0; j < param.length(); j++) {
          param.data[j] -=
            (this.lr * momentumsCorrected[j]) /
            (Math.sqrt(velocitiesCorrected[j]) + this.epsilon);
        }
      }
    }
  }
...
export class RMSProp extends Optimizer {
  ...
  override step() {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      this.squared_gradients[i] = this.squared_gradients[i].map(
        (squared_gradient, j) => {
          squared_gradient =
            this.rho * squared_gradient +
            (1 - this.rho) * param.grad[j] * param.grad[j];
          param.data[j] -=
            (this.lr * param.grad[j]) /
            (Math.sqrt(squared_gradient) + this.epsilon);
